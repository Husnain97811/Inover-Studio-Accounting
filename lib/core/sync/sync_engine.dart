...
class SyncEngine {
  ...
  Future<void> _pushOutbox() async {
    final pending =
        await (_db.select(_db.outboxQueue)
              ..where((t) => t.syncedAt.isNull())
              ..orderBy([(t) => OrderingTerm.asc(t.createdAt)])
              ..limit(AppConstants.outboxBatchSize))
            .get();
    ...
    await (_db.delete(_db.outboxQueue)..where(
          (t) => t.syncedAt.isNotNull() & t.syncedAt.isSmallerThanValue(cutoff),
        ))
        .go();
  }
  ...
  Future<void> _pullEntity(String entity) async {
    ...
    try {
      ...
      for (final row in rows) {
        await _upsertRow(entity, row as Map<String, dynamic>);
      }

      if (rows.isNotEmpty) {
        final maxTs = rows
            .map((r) => (r as Map)['updated_at'] as int? ?? 0)
            .reduce((a, b) => a > b ? a : b);
        await _db
